...
# 3.キーワード引数(引数の数が多くなった時に、それぞれの値がどの引数に入るのか分かりやすくする記述)
# 通常のメソッドの書き方に加えて、①定義側で、引数の後にコロンを付ける。②呼び出し側で、値の前に引数名を書く。
...
# →テレビを2台のお買い上げです 合計金額は30000円です



# 4.戻り値(呼び出し元で受け取る処理結果のこと。jsとほぼ変わらない、詳しくはjs3のリポジトリ参照)
...
def price_with_shipping(price)
  # ↓priceが5000以上(true)の場合はif文内のreturnが実行され処理が終了するが、priceが5000以下(false)の場合はif文内のreturnが実行されずif文後のreturnが実行され処理が終了する
  if price >= 5000
    return price
  end
...
puts "お支払い金額は、送料込みで#{price_with_shipping(3000)}円です" # priceが5000以下(false)、if文内のreturnが実行されずif文後のreturnが実行され処理が終了
puts "-----------"
puts "商品の合計金額は10000円です"
puts "お支払い金額は、送料込みで#{price_with_shipping(10000)}円です" # priceが5000以上(true)、if文内のreturnが実行され処理が終了
# →商品の合計金額は3000円です　お支払い金額は、送料込みで3500円です ----------- 商品の合計金額は10000円です お支払い金額は、送料込みで10000円です